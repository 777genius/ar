...
export function registerCodexGoalPrompts(server: McpServer): void {
  for (const prompt of [
...
function codexGoalPromptText(name: string, jobId: string | undefined): string {
  ...
  if (name === "start_codex_goal_worker") {
    return `${shared} First call codex_goal_decision. Start or continue only when decision.safeToContinue is true, otherwise follow decision.checklist and decision.nextBestCommand. If no job exists yet, create one with model gpt-6-astra, reasoningEffort high, serviceTier default, app-server-goal behavior and 72h timeout.`;
  }
  if (name === "monitor_codex_goal_worker") {
    return `${shared} Use codex_goal_overview once for pool context, then poll codex_goal_brief in compact mode. Pass each revision back as afterRevision and back off polling while unchanged. Do not repeatedly fetch overview, decision, accounts or logs unless compact state changes or action is required. Call codex_goal_decision immediately before taking action. If worker is alive and silentStale is false, keep monitoring instead of starting another worker. If silentStale is true, request a log tail and verify progress heartbeat, tmux, runner process, app-server process and git status before stopping or recovery.`;
  }
