...
const MAX_CONCURRENT_ORPHAN_OBSERVATIONS = 8;

export type LedgerEpochOrphanWorkspaceBindingRevalidation =
...
export async function bindLedgerEpochOrphanWorkspace(input: {
  ...
  /** Observation failures must remain distinct while checking a sealed binding. */
  readonly failOnObservationError?: boolean;
  ...
  let head: string;
  try {
    const identity = (await gitBytes(canonicalPath, [
      "rev-parse",
      "--show-toplevel",
      "HEAD",
    ...
    if (input.failOnObservationError) throw error;
    return undefined;
  }
  ...
  const untracked = nulValues(status)
  ...
  const binding: ConsumedOutputLedgerEpochOrphanWorkspaceBinding = {
    declaredPath,
    state: "quarantined",
    canonicalPath,
    device: metadata.dev,
...
export async function bindLedgerEpochOrphanWorkspaces(input: {
  readonly workspacePaths: readonly string[];
  readonly requiredWorkspacePaths: ReadonlySet<string>;
  readonly deniedRoots: readonly string[];
}): Promise<readonly ConsumedOutputLedgerEpochOrphanWorkspaceBinding[]> {
  const bindings = new Array<
    ConsumedOutputLedgerEpochOrphanWorkspaceBinding | undefined
  >(input.workspacePaths.length);
  let nextIndex = 0;
  const observe = async (): Promise<void> => {
    while (nextIndex < input.workspacePaths.length) {
      ...
      if (!binding && input.requiredWorkspacePaths.has(resolve(workspacePath))) {
        throw new Error("ledger_epoch_clean_or_missing_orphan_refused");
      }
      bindings[index] = binding;
    }
  };
  await Promise.all(Array.from(
    { length: Math.min(MAX_CONCURRENT_ORPHAN_OBSERVATIONS, input.workspacePaths.length) },
    observe,
  ));
  return bindings.filter(
    (binding): binding is ConsumedOutputLedgerEpochOrphanWorkspaceBinding =>
      binding !== undefined,
  );
}
...
export async function ledgerEpochOrphanWorkspaceBindingMatches(input: {
  ...
  return (await revalidateLedgerEpochOrphanWorkspaceBinding(input)).matches;
}

export async function revalidateLedgerEpochOrphanWorkspaceBinding(input: {
  readonly binding: ConsumedOutputLedgerEpochOrphanWorkspaceBinding;
  readonly deniedRoots: readonly string[];
}): Promise<LedgerEpochOrphanWorkspaceBindingRevalidation> {
  try {
    const current = await bindLedgerEpochOrphanWorkspace({
      workspacePath: input.binding.declaredPath,
      deniedRoots: input.deniedRoots,
      failOnObservationError: true,
    });
    return JSON.stringify(current) === JSON.stringify(input.binding)
      ...
      : {
          matches: false,
          category: "mismatch",
          path: input.binding.declaredPath,
        };
  } catch {
    return {
      matches: false,
