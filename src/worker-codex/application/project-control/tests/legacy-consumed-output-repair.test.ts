...
describe("legacy consumed-output repair", () => {
  it("previews without writes, bulk quarantines exact bytes, and replays idempotently", async () => {
    ...
    try {
      ...
      const replaySnapshot = await snapshotFromLedger(
        fixture.ledgerRoot,
        fixture.backupRoot,
      );
  ...
  it("refuses an authored retained patch and keeps admission blocked", async () => {
    ...
    try {
      ...
      await expect(access(fixture.paths[0]!)).resolves.toBeUndefined();
      expect(admission(await snapshotFromLedger(
        fixture.ledgerRoot,
        fixture.backupRoot,
      )).allowed).toBe(false);
    } finally {
  ...
  it("refuses future schemas and job ids outside the project prefix", async () => {
    ...
    try {
      ...
      const futureSnapshot = await snapshotFromLedger(
        fixture.ledgerRoot,
        fixture.backupRoot,
      );
      ...
      const foreign = await repairLegacyConsumedOutputDebt({
        ...
        admissionSnapshot: await snapshotFromLedger(
          fixture.ledgerRoot,
          fixture.backupRoot,
        ),
        mutationLocks: fixture.locks,
...
async function makeFixture(count: number) {
  ...
  return {
    ...
    paths,
    backupRoot,
    snapshot: await snapshotFromLedger(ledgerRoot, backupRoot),
    locks: new TestLocks(),
  };
}
...
async function snapshotFromLedger(
  ledgerRoot: string,
  evidenceRoot: string,
