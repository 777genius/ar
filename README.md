...
- npm consumers using GitHub dependencies expect package-lock reproducibility;
- Docker builds should not depend on each consumer compiling this package from
  TypeScript during install.

Runtime design response:

- one public GitHub-installable package;
- subpath exports for clean module boundaries;
- committed `dist` for reliable GitHub dependency installs;
...
miss, such as missing `dist` files, broken `exports`, or imports that only work
because the source tree is nearby.
...
- `dist` is committed so npm and Docker consumers do not need to compile the
  library during install;
- package-lock consumers still pin an exact Git commit, even when the dependency
  points at `main`;
- CI has to verify that `dist` matches `src`;
